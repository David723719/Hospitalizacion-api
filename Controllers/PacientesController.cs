...
public class PacienteDto { public string Codigo { get; set; } = ""; public string? Nombre { get; set; } public DateTime FechaNacimiento { get; set; } }using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using HospitalizacionAPI.Data;
using HospitalizacionAPI.Models;
...
public class PacientesController : ControllerBase
{
    ...
    public async Task<IActionResult> Crear([FromBody] PacienteDto dto)
    {
        ...
        _db.Pacientes.Add(paciente);
        await _db.SaveChangesAsync();
        return CreatedAtAction(nameof(Listar), new { codigo = paciente.Codigo }, new { mensaje = "Creado" });
    }
